...
function analyzeDocument(document) {
    ...
    const parseFunction = (startLine, isMethod = false, className = '') => {
 ...
 */
function getAnalyzedDocument(document) {
...
const completionProvider = vscode.languages.registerCompletionItemProvider('dark', {
    provideCompletionItems(document, position, token, context) {
        ...
        if (importMatch) {
            ...
            if (position.character > quoteStartIndex) {
                ...
                return Object.keys(standardLibrary).map(name => new vscode.CompletionItem(name, vscode.CompletionItemKind.Module));
            }
        }
        ...
        if (memberAccessMatch) {
            ...
            if (objectName in standardLibrary) {
                const module = standardLibrary[objectName];
            ...
            const addedMethodsFromClasses = new Set();
        ...
        const keywordCompletions = keywords.map(keyword => new vscode.CompletionItem(keyword, vscode.CompletionItemKind.Keyword));
        ...
        const constantCompletions = constants.map(c => new vscode.CompletionItem(c, vscode.CompletionItemKind.Constant));
        // Анализ текущего файла
        const analysis = getAnalyzedDocument(document);
        // Определяем, в какой функции мы находимся
        const currentFunction = analysis.functions.find(f => position.line >= f.startLine && position.line <= f.endLine);
        ...
        const importCompletions = analysis.imports.map(imp => new vscode.CompletionItem(imp.name, vscode.CompletionItemKind.Module));
...
const hoverProvider = vscode.languages.registerHoverProvider('dark', {
    provideHover(document, position, token) {
        ...
        if (match) {
            ...
            const matchingMethods = analysis.classes.flatMap(cls => cls.methods.filter(m => m.name === memberName));
        ...
        if (word in builtInFunctions) {
            const funcData = builtInFunctions[word];
            const content = new vscode.MarkdownString();
    ...
    provideDocumentSemanticTokens(document) {
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            while ((multiMatch = multiStringRegex.exec(lineTextForAnalysis)) !== null) {
                if (!inMultiLineString) {
                    inMultiLineString = multiMatch[0];
...
function updateDiagnostics(document, collection) {
    ...
    (0, child_process_1.execFile)(executorPath, ['--check', document.uri.fsPath], (err, stdout, stderr) => {
...
function activate(context) {
    ...
    const runCommand = vscode.commands.registerCommand('dark.run', () => {
        ...
        const executorPath = config.get('executorPath');
        if (!executorPath) {
            vscode.window.showErrorMessage('Path to Dark executor is not set. Please set "dark.executorPath" in your settings.');
            return;
        }
    ...
    const debouncedUpdateDiagnostics = debounce((doc) => {
        updateDiagnostics(doc, diagnosticCollection);
    }, 500); // Задержка в 500 мс
    if (vscode.window.activeTextEditor) {
        updateDiagnostics(vscode.window.activeTextEditor.document, diagnosticCollection);
    }
    context.subscriptions.push(vscode.workspace.onDidOpenTextDocument(doc => updateDiagnostics(doc, diagnosticCollection)), vscode.workspace.onDidChangeTextDocument(event => {
        if (event.document.languageId === 'dark') {
            debouncedUpdateDiagnostics(event.document);
        }
    }), vscode.workspace.onDidCloseTextDocument(doc => diagnosticCollection.delete(doc.uri)));
    context.subscriptions.push(runCommand, completionProvider, hoverProvider, vscode.languages.registerDocumentSemanticTokensProvider('dark', semanticTokensProvider, legend));
}
function deactivate() {
    if (darkTerminal) {
